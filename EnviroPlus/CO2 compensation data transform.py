...
measured_data['timestamp'] = measured_data['timestamp'].apply(lambda x: datetime.strptime(x, r"%Y-%m-%d %H:%M:%S.%f").timestamp())
calibration_data['Time'] = calibration_data['Time'].apply(lambda x: datetime.strptime(x, '%d/%m/%Y %H:%M:%S').timestamp())
...
correction_factor = sum(calibration_data_interp) / sum(measured_data['co2_ppm'])

# Apply the correction factor to the measured data
calibrated_data = measured_data['co2_ppm'] * correction_factor

# Save the calibrated data to a new file
calibrated_data.to_csv('calibrated_data.csv', index=False)
